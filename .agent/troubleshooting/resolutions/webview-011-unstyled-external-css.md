...
`build-and-install.ps1` can rebuild VST3 while a running Standalone **locks `Organik.exe`**. Next launch still uses the old unstyled HTML. Verify both binaries contain a unique HTML marker. If VST3 has it and EXE does not: kill the process and rebuild Standalone.
...
Do **not** funnel every miss to `index.html`. Map known files; return `nullopt` otherwise. Match `css/tokens.css` **or** basename `tokens.css`. Confirm names in `BinaryData.h`.

### C. Plugin-host layout

When `window.__JUCE__` exists, add `html.plugin-host`, hide `.hint`, scale with pad 1.0.

### D. Rebuild both formats

Stop Standalone, run `.\scripts\build-and-install.ps1 -PluginName <Name> -SkipTests`, confirm Standalone **and** VST3 timestamps.
...
**Canonical copy:** `.claude/troubleshooting/resolutions/webview-011-unstyled-external-css.md`

---

## Related issues

- **webview-008:** ES6 modules / external JS fail
- **webview-007:** Black screen — BinaryData
- **webview-010:** Black screen — resource provider not called
