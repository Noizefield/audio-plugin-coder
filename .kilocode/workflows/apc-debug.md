...
# Debug Phase
**Preferred model:** Read `apc.config.json` -> `models.phases.debug` (announce to user; switch host model if possible).
...
```
≡ƒÉ₧ Debugging complete!

Root cause identified: [Yes/No]
Failure reproducible: [Yes/No]
Confidence score: [0.0ΓÇô1.0]

Next step:
- /fix [Name] if root cause is identified
- /apc-debug [Name] to continue investigation
```
