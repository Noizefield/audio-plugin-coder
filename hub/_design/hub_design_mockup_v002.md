# APC Hub — Mockup v002 (review record)

Spec: `hub_design_spec_v002.md` · Status: DRAFT · Snapshot baked: 2026-09-13
Twin runnable file: `hub_design_mockup_v002.html` (same folder — open via
`file://`, zero deps, system fonts, no images).
Changes vs v001: dark theme default + light alternate (switch top-right,
persisted), phosphor green for all OK/good elements, amber strictly for
active/warning. Data snapshot unchanged.

## How to review

1. Open `hub_design_mockup_v002.html` — it must load DARK first.
2. Flip `THEME: [*DARK] [LIGHT]` both ways; reload — choice must persist.
3. Check green discipline: green only on `[OK]`/`[x]`/healthy; amber only on
   current-phase chips, `[!!]`, logo arrow.
4. Reply with what to change → `_v003` (+ CHANGELOG entry).

## Section map (unchanged from v001)
...
<title>APC HUB v002 // mockup — snapshot 2026-09-13</title>
<style>
/* == themes.css (future split): dark is default, light is alternate == */
...
.mono,pre,table,code,.kicker,.nav,.theme{font-family:Consolas,"Cascadia Mono",Menlo,monospace;}
...
.badge.ok{background:var(--good); border-color:var(--good); color:var(--good-ink); font-weight:bold;}
/* == components.css == */
.theme{font-size:12px; margin:0; white-space:nowrap; padding-top:4px;}
...
<p class="titleline">APC HUB <span class="mut">// v002 mockup — read-only command center</span></p>
</div>
...
<span class="good">[OK]</span> ../apc_plugins|builds|releases      -&gt; all exist on disk</pre>
</section>
</main>

<footer class="mono">
=== APC HUB v002 ============================================================<br>
snapshot baked 2026-09-13 · dark default · amber = active/warn · phosphor green = ok/good · theme switch top-right (persisted) · zero deps, system fonts, no images
</footer>

</div>
<!-- theme switch: the single JS exception allowed by spec v002 §5.9 -->
<script>
(function(){
...
1. Phosphor `#33FF33` / light `#0B7A30` — keep or adjust shade?
2. Green on every `[x]`/`[OK]` — right amount, or too loud in dark mode?
3. Kicker numbers: keep monochrome or amber in dark mode?
4. Any value wrong vs live tree?
