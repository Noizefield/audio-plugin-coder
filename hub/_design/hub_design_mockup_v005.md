# APC Hub - Mockup v005 (review record)

Spec: `hub_design_spec_v005.md` · Status: DRAFT · Snapshot baked: 2026-09-13
Twin runnable file: `hub_design_mockup_v005.html` (same folder - open via
`file://`, zero deps, system fonts, no images).
Changes vs v004: persistent top header (logo + title + badges) rendering on
EVERY view - the logo no longer lives inside 01 and can no longer disappear.
Sidebar slimmed (version-only mark, REL, nav, theme, foot). Data, tokens,
switching unchanged.

## How to review

1. Open `hub_design_mockup_v005.html`, click through all sidebar items -
   logo + title + badges stay put on every view.
2. Confirm 01 OVERVIEW now starts directly with its kicker and reads fine
   under the permanent header.
3. Reply with what to change → `_v006` (+ CHANGELOG entry).
...
<title>APC HUB v005 // mockup - snapshot 2026-09-13</title>
<style>
...
a{color:var(--ink);}
/* == shell.css: centered frame + persistent header + sidebar + switching == */
...
<p class="mono badges"><span class="badge ok">[OK] SETUP 2026-08-14 / WINDOWS</span><span class="badge">PROFILE: BALANCED</span><span class="badge">SNAP 2026-09-13</span></p>
</div>
</header>
...
<pre class="mark">+--------------------+
| APC v1.4.0         |
| <span class="mut">v005 · DRAFT</span>       |
+--------------------+</pre>
...
<footer class="mono">
=== APC HUB v005 ============================================================<br>
snapshot baked 2026-09-13 · persistent header · centered shell · APC v1.4.0 + update-check spec §10 · dark default · amber = active/warn · phosphor green = ok/good · zero deps, system fonts, no images
</footer>
</div>
...
1. Header static (v005) or scroll-sticky?
2. Badges in header vs back in sidebar?
3. Sidebar density right now?
